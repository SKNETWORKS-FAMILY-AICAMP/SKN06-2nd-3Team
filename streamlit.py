...
CurrentEquipmentDays = st.sidebar.number_input('현재 장비 사용 일수', min_value=0.0, value=0.0)
HandsetModels = st.sidebar.number_input('휴대폰 모델 수', min_value=0, value=0)
IncomeGroup = st.sidebar.number_input('소득 그룹', min_value=0, max_value=9, value=0)

# 범주형
children_in_HH = ["Yes", "No"]
...
# 컬럼별 선택 위젯
ChildrenInHH = st.sidebar.selectbox('가구 내 자녀 수:', children_in_HH)
